...
&\bx_t^{(\bl^{max})} = (\tilde{x}_{t,1}, \ldots, B^{(l^{max}_1)} \tilde{x}_{t,1}, \ldots, \tilde{x}_{t,D}, \ldots, B^{(l^{max}_D)} \tilde{x}_{t,D})
\end{align*}
...
\begin{align}
&\widehat{f}(\by_{t^*}^{(P)} | \bx_{t^*}) = \sum_{t \in \tau} w_{t, t^*} K^{pred}(\by_t^{(P)}, \by_{t^*}^{(P)} ; H^{pred}) \text{, where} \label{eqn:PredLine1} \\
...
\begin{align*}
&K(x, x^* ; h) \varpropto \exp\left\{ -\frac{1}{2} sin^2\left(\rho \frac{x - x^*}{h}\right) \right\}
\end{align*}
...
\begin{align*}
&(\widehat{\bu}, \widehat{H}^{wt}) \approx \argmin{(\bu, H^{wt})} MAE(\bu, H^{wt}) \\
&\qquad = \argmin{(\bu, H^{wt})} \frac{1}{P \cdot (1 + \max_d \bl^{max})} \sum_{p = 1}^{P} \sum_{t^* = 1 + \max_d \bl^{max}}^{T - P} \vert \widehat{y}_{t^*}^{(p)} - y_{t^*}^{(p)} \vert
\end{align*}
...
In order to obtain the point estimate $\widehat{y}_{t^*}^{(p)}$ for each time point $t^*$ and number of steps ahead $p$, we first form the predictive density as in Equations \eqref{eqn:PredLine1} through \eqref{eqn:PredLine4}.  In order to reduce the potential for our parameter estimates to be affected by local correlation in the time series, we eliminate all time points that fall within one year of $t^*$ from the index set $\tau$; we refer to this timepoint-specific index set as $\tau^*$.  We then take our point estimate to be the expected value of this estimated predictive distribution:
\begin{align*}
&\widehat{\by}_{t^*}^{(P)} = \mathbb{E} \left[ \sum_{t \in \tau^*} w_{t, t^*} K^{pred}(\by_t^{(P)}, \by_{t^*}^{(P)} ; H^{pred}) \right] \\
&\qquad = \sum_{t \in \tau^*} w_{t, t^*} \mathbb{E} \left[ K^{pred}(\by_t^{(P)}, \by_{t^*}^{(P)} ; H^{pred}) \right] \\
&\qquad = \sum_{t \in \tau^*} w_{t, t^*} \by_t^{(P)}
\end{align*}
