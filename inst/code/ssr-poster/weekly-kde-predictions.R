...
for(predictions_df_row_ind in seq_len(nrow(predictions_df))) {
	...
	new_data <- San_Juan_test[seq_len(last_obs_ind), , drop=FALSE]
	## add log column
	...
	obs <- San_juan_test$total_cases[last_obs_ind + ph, ssr_fit$y_names]
	log_obs <- log(obs + 1)
	...
	predictions_df$prediction[predictions_df_row_ind] <- expected_value_exp_norm_mixt(ssr_prediction$pred_centers_and_weights$log_weights, ssr_prediction$pred_centers_and_weights$centers[, 1], ssr_fit$predictive_bw)
	...
	predictions_df$log_score[predictions_df_row_ind] <- logspace_sum(ssr_prediction$pred_centers_and_weights$log_weights + 
		sapply(ssr_prediction$pred_centers_and_weights$centers, function(center) {dnorm(log_obs, mean = log(center + 1), sd = ssr_fit$predictive_bw, log=TRUE)})) -
		log(obs + 1)
